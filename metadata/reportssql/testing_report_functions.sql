...
DROP FUNCTION IF EXISTS getANCDate //

CREATE FUNCTION getANCDate(encounter_id INT) 
RETURNS DATE
DETERMINISTIC
BEGIN
    DECLARE result DATE;
    SELECT DATE(encounter_datetime) INTO result FROM encounter 
    WHERE encounter_id = encounter_id AND voided = 0
    LIMIT 1;
    RETURN result;
END $$
